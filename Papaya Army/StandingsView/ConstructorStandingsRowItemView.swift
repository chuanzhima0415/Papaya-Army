...
struct ConstructorStandingsRowItemView: View {
	...
	var body: some View {
		HStack {
			switch constructorStanding.position {
			case 1:
			...
			default:
				Image(systemName: "\(constructorStanding.position).circle")
					.imageScale(.large)
			}
			HStack {
				VStack {
					Text("\(constructorStanding.teamName)")
				}
				Spacer()
				VStack {
					Text("\(constructorStanding.points)")
					Text("pts")
				}
			}
...
#Preview {
	ConstructorStandingsRowItemView(constructorStanding: ConstructorStanding(teamId: "red_bull", points: 188, position: 1))
}
